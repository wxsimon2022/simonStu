...
-- 2.1 目录（顶级分组）
INSERT IGNORE INTO c_permissions (name, description, type) VALUES
('user',   '用户管理', 'dir'),
('system', '系统管理', 'dir');

-- 2.2 菜单页（二级节点，parent → dir）
--     命名规范: xxx:list，路由自动解析为 /{xxx}
INSERT INTO c_permissions (name, description, type, parent_id) VALUES
('user:list',  '用户列表', 'menu',
  (SELECT id FROM (SELECT id FROM c_permissions WHERE name = 'user') AS tmp)),
('admin:list', '管理员管理', 'menu',
  (SELECT id FROM (SELECT id FROM c_permissions WHERE name = 'system') AS tmp)),
('role:list',  '角色管理', 'menu',
  (SELECT id FROM (SELECT id FROM c_permissions WHERE name = 'system') AS tmp)),
('perm:list',  '菜单管理', 'menu',
  (SELECT id FROM (SELECT id FROM c_permissions WHERE name = 'system') AS tmp))
ON DUPLICATE KEY UPDATE description = VALUES(description), type = VALUES(type), parent_id = VALUES(parent_id);
